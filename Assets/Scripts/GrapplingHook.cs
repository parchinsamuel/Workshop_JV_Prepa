...
public class GrapplingHook : MonoBehaviour
{
    ...
    void LancerRayon()
    {
        ...
        if (Physics.Raycast(ray, out hit, DistanceMax, HangingLayers))
        {
            ...
            Vector3 directionBalancier = Vector3.ProjectOnPlane(Camera.main.transform.forward, directionCorde);

            if (directionBalancier.sqrMagnitude == 0)
            {
                directionBalancier = Camera.main.transform.up;
            }

            _rb.AddForce(directionBalancier.normalized * forceImpulsionInitiale, ForceMode.Impulse);
